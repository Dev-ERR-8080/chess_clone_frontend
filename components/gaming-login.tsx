...
const LoginForm: React.FC<LoginFormProps> = ({ onSubmit }) => {
    ...
    function handleSteamLogin(): void {
        ...
        <><div className="p-8 rounded-2xl backdrop-blur-sm bg-black/50 border border-white/10">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="mt-6 grid grid-cols-3 gap-3">
                    <SocialButton icon={<ChromeIcon size={18} />} name="Chrome" onClick={handleChromeLogin} />
                    <SocialButton icon={<Twitter size={18} />} name="X" onClick={handleTwitterLogin} />
                    <SocialButton icon={<Gamepad2 size={18} />} name="Steam" onClick={handleSteamLogin} />
                </div>
            </div><p className="mt-8 text-center text-sm text-white/60">
                Don't have an account?{' '}
                <a href="#" className="font-medium text-white hover:text-purple-300 transition-colors" onClick={()=> setIsRegister(!isRegister)}>
                    Create Account
                </a>
            </p></>
    );
